        ...
        failure {
            script {
                withCredentials([string(credentialsId: 'discord-build-status', variable: 'DISCORD_BUILD_STATUS')]) {
                    ...
                    // Capture console output, but limit to first 3000 characters to avoid Discord message size limits
                    def consoleOutput = sh(
                        script: 'cat ${JENKINS_HOME}/jobs/${JOB_NAME}/builds/${BUILD_NUMBER}/log | head -c 3000',
                        returnStdout: true
                    ).trim()
                    
                    // Escape special characters in console output
